...
stateW w m = do
    ...
    return $ fmap (const (extract w,b)) $ adjEnv a (void $ lower w)
{-
...
stateWFst f w m = do
    ...
    return $ fmap fst $ wab @## (adjEnv (snd $ extract wab) (lower w))
...
stateWSnd f w m = do
    ...
    return $ fmap snd $ (adjEnv (fst $ extract wab) (lower w)) @## wab
-}
