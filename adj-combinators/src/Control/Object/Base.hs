...
adjObjComp (wv1 :< wo1) (wv2 :< wo2) = (wv1 @## wv2) :< (fmap adjObjComp $ wo1 @## wo2)
...
asjObjComb (wv1 :< wo1) (wv2 :< wo2) = do
   wv <- (wv1 @+* wv2) 
   wo <- ((fmap . fmap) adjObjComp $ wo1 @+* wo2)
   return $ wv :< wo
...
viewObject :: 
   (W.AdjointT fo go w () -> (W.AdjointT fv2 gv2 w (), W.AdjointT fo go w ())) ->
   W.AdjointT fo go w () ->
   AdjObject w fo go fv gv ->
